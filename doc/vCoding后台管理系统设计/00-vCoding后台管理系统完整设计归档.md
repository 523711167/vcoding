...
> 归档范围：本稿已合并 `doc/需求变更/` 中截至 `2026-04-09` 的已确认变更，包括用户组织展开、角色数据权限展开、`data_scope` 字符串化、菜单类型编码化、内置管理员保护、流程定义原始 JSON、流程定义版本发布规则、流程定义移除 `biz_code`、流程节点分钟口径、审批组织展开、运行层字段补充、默认分支规则、并行归属字段、业务申请 `biz_name` 冗余、并行作用域模型、代办箱查询、已办箱查询、流程加签、登录日志、查询箱同组织可见、发起人取消流程等。
...
| 场景 | 建议优先阅读章节 |
|------|------------------|
| 快速理解系统全貌 | [三、总体设计](#三总体设计) → [四、核心概念](#四核心概念) → [五、业务、流程与权限关系](#五业务流程与权限关系) |
| 落表或补 SQL | [六、系统基础数据设计](#六系统基础数据设计) → [八、工作流定义层设计](#八工作流定义层设计) → [九、工作流运行层设计](#九工作流运行层设计) |
| 做权限与管理后台开发 | [七、权限设计](#七权限设计) → [十二、后台管理功能设计](#十二后台管理功能设计) → [十三、接口设计概要](#十三接口设计概要) |
| 做流程引擎开发 | [八、工作流定义层设计](#八工作流定义层设计) → [九、工作流运行层设计](#九工作流运行层设计) → [十、工作流核心运行机制](#十工作流核心运行机制) |
| 校验边界与联动规则 | [十四、与审批工作流的衔接规则](#十四与审批工作流的衔接规则) → [十五、边界条件与校验规则](#十五边界条件与校验规则) → [十六、注意事项与扩展点](#十六注意事项与扩展点) |
...
- [三、总体设计](#三总体设计)
- [四、核心概念](#四核心概念)
- [五、业务、流程与权限关系](#五业务流程与权限关系)
- [六、系统基础数据设计](#六系统基础数据设计)
- [七、权限设计](#七权限设计)
- [八、工作流定义层设计](#八工作流定义层设计)
- [九、工作流运行层设计](#九工作流运行层设计)
- [十、工作流核心运行机制](#十工作流核心运行机制)
- [十一、状态机设计](#十一状态机设计)
- [十二、后台管理功能设计](#十二后台管理功能设计)
- [十三、接口设计概要](#十三接口设计概要)
- [十四、与审批工作流的衔接规则](#十四与审批工作流的衔接规则)
- [十五、边界条件与校验规则](#十五边界条件与校验规则)
- [十六、注意事项与扩展点](#十六注意事项与扩展点)
- [十七、开发落地建议](#十七开发落地建议)
...
### 6.1 业务申请表 `tb_biz_apply`

用一张通用表覆盖所有业务申请场景（请假、报销、合同等），通过 `biz_definition_id` 关联业务定义，差异化字段存入 `form_data` JSON 列。
...
### 6.5 业务定义角色关联表 `tb_biz_definition_role_rel`
...
### 6.10 用户-部门关联表 `tb_user_dept_rel`
...
#### 6.10.1 用户-组织展开关系表 `tb_user_dept_rel_expand`
...
### 6.12 角色-菜单关联表 `tb_user_role_menu`
...
### 6.13 角色-自定义数据权限部门表 `tb_user_role_dept`
...
#### 6.13.1 角色-数据权限组织展开表 `tb_user_role_dept_expand`
...
- 不应用于误拦截用户自己的资料、自己的申请单、自己的待办任务、自己的已办任务
...
### 8.1 流程定义表 `tb_workflow_definition`
...
CREATE TABLE `tb_workflow_node` (
  ...
  `parallel_split_node_id` BIGINT                         COMMENT '所属最近一层并行拆分节点定义ID，非并行段为空',
...
### 8.3 节点审批人配置表 `tb_workflow_node_approver`
...
- `approver_value` 统一使用 `BIGINT` 保存单个审批主体主键
...
### 8.3.1 节点审批组织展开表 `tb_workflow_node_approver_dept_expand`
...
### 8.4 流转条件表 `tb_workflow_transition`
...
CREATE TABLE `tb_workflow_transition` (
  ...
  `from_node_name`  VARCHAR(100) NOT NULL               COMMENT '来源节点名称（冗余）',
  `from_node_type`  VARCHAR(32)  NOT NULL               COMMENT '来源节点类型（冗余）',
  `to_node_id`      BIGINT       NOT NULL               COMMENT '目标节点ID',
  `to_node_name`    VARCHAR(100) NOT NULL               COMMENT '目标节点名称（冗余）',
  `to_node_type`    VARCHAR(32)  NOT NULL               COMMENT '目标节点类型（冗余）',
...
- `from_node_name` / `from_node_type`：来源节点名称与类型冗余
- `to_node_id`：表示流转到哪个目标节点
- `to_node_name` / `to_node_type`：目标节点名称与类型冗余
...
### 9.1 流程实例表 `tb_workflow_instance`
...
CREATE TABLE `tb_workflow_instance` (
  ...
  `current_node_id`   BIGINT                              COMMENT '当前所在节点定义ID',
  `current_node_name` VARCHAR(100)                        COMMENT '当前所在节点定义名称（冗余）',
  `current_node_type` VARCHAR(32)                         COMMENT '当前所在节点定义类型（冗余）',
...
CREATE TABLE `tb_workflow_node_instance` (
  ...
  `definition_node_id` BIGINT      NOT NULL            COMMENT '对应的节点定义ID',
...
### 9.4 审批操作记录表 `tb_workflow_approval_record`
...
- `tb_workflow_node_instance.definition_node_*` 统一表示节点实例对应的节点定义快照
...
- 当前仅支持“前加签”，且加签审批人仍归属当前活动节点，不新增节点实例
- 发起加签时，来源审批人实例更新为 `WAITING_ADD_SIGN` 且 `is_active=0`
- 新增的加签审批人实例写入当前节点，统一标记 `relation_type=ADD_SIGN`
- 多个加签审批人按 `addSignUserIds` 顺序创建并顺签，首位加签人立即激活，其余先置为未激活
- 加签链全部通过后，来源审批人恢复为 `PENDING` 且 `is_active=1`，节点继续等待来源审批人最终审批
- 任一加签审批人拒绝后，其余未完成加签审批人统一取消，当前处于 `WAITING_ADD_SIGN` 的来源审批人同步取消，节点按拒绝结束
...
加签补充口径：
- 仅支持当前审批人在当前 `ACTIVE` 节点上对自己的审批席位发起“前加签”
- 发起加签时要求流程实例为 `RUNNING`，节点实例属于当前流程实例且节点状态为 `ACTIVE`
- 仅允许当前用户操作自己名下 `status=PENDING` 且 `is_active=1` 的审批人实例
- 不允许给自己加签，不允许加签给当前节点审批链中已存在的用户，同一次请求中的加签用户也不允许重复
- 加签目标用户必须存在且处于启用状态
- 发起成功后写入一条 `ADD_SIGN` 审批记录，`comment` 保存加签说明，`extra_data` 保存本次加签用户 ID 列表
...
### 13.8 流程定义模块 `/sys/workflow-definition`
...
| `/sys/biz-apply/processed/list` | GET | 查询已办箱列表 |
| `/sys/biz-apply/processed/page` | GET | 分页查询已办箱 |
| `/sys/biz-apply/processed/detail` | GET | 查询已办详情 |
...
- 已办箱统一按 `finished_at DESC, id DESC` 排序，`list/page/detail` 共用 `bizApplyId`、`bizDefinitionId`、`title` 过滤条件和同一套查询逻辑
- 已办箱返回结构复用代办箱字段，并额外返回 `processedAt` 表示审批人实例处理完成时间；原有 `todoAt` 继续表示待办进入时间
...
- 已办箱同样按“当前用户是否真实处理过该审批任务”判断可见性，不再额外套用普通业务数据权限口径
...
- 查询箱、代办箱、已办箱、我的发起的 `list/page/detail` 必须保持各自模块内查询条件、排序和字段结构一致
...
- 加签仅允许发生在流程实例 `RUNNING`、节点实例 `ACTIVE`、且当前用户本人对应审批人实例 `status=PENDING` 且 `is_active=1` 的场景
- 不允许给自己加签，不允许加签给当前节点审批链中已存在的用户，同一次加签请求中的用户 ID 也不允许重复
- 加签目标用户必须为存在且启用中的系统用户
- 来源审批人处于 `WAITING_ADD_SIGN` 期间不进入代办箱；加签链全部通过后恢复待办；若加签链被拒绝，则来源审批人不再继续审批
- 已办箱仅统计 `APPROVED`、`REJECTED`、`DELEGATED`，`WAITING_ADD_SIGN` 与 `CANCELED` 不纳入已办
