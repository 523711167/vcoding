...
CREATE TABLE IF NOT EXISTS `tb_biz_apply` (
  ...
  `biz_status` VARCHAR(16) NOT NULL DEFAULT 'DRAFT' COMMENT '业务申请状态：DRAFT=草稿 PENDING=审批中 APPROVED=已通过 REJECTED=已拒绝 CANCELED=已撤回 INITIATOR_CANCELED=已取消',
...
CREATE TABLE IF NOT EXISTS `tb_workflow_instance` (
  ...
  `status` VARCHAR(16) NOT NULL DEFAULT 'RUNNING' COMMENT '实例状态：RUNNING=进行中 APPROVED=已通过 REJECTED=已拒绝 CANCELED=已撤回 INITIATOR_CANCELED=已取消',
  `applicant_id` BIGINT NOT NULL COMMENT '申请人用户ID',
  `applicant_name` VARCHAR(64) NOT NULL COMMENT '申请人姓名（冗余）',
  `form_data` JSON DEFAULT NULL COMMENT '业务申请数据快照',
