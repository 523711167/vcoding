 ...
 */
@Data
@Schema(description = "流程节点参数")
public class WorkflowDefinitionNodeETO {

    @Schema(description = "节点编码（定义内唯一）", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "节点类型：START/APPROVAL/CONDITION/PARALLEL_SPLIT/PARALLEL_JOIN/END", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "nodeType不能为空")
    @EnumCodeValid(enumClass = WorkflowNodeTypeEnum.class, allowNull = false, message = "nodeType不合法")
    private String nodeType;

    @Schema(description = "审批模式：AND/OR/SEQUENTIAL")
