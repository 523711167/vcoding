...
- 任意数据库变更都必须新增当天日期的 SQL 文件；哪怕只是字段注释变化，也要新增 `sql/yyyyMMdd.sql` 记录本次变更。
- `init.sql` 记录当前最新数据库结构；字段、索引、注释等发生变化时，必须同步更新 `init.sql`。
- 除 `init.sql` 和当天新增的 `sql/yyyyMMdd.sql` 外，历史日期 SQL 文件一律不修改。
- 用户和角色和组织关系的DML已经固定，后续菜单新增，需要同步更新 `init.sql` 中对应DML语句。
