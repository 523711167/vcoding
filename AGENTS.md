...
- 测试代码：`src/test/java`
- 初始化 SQL：`sql/`
- 设计与说明文档：`README.md`、`doc/README.md`、`doc/vCoding后台管理系统设计/`、`doc/需求变更/`
- 构建产物：`target/`，禁止手工修改
...
## 文档变更规则
- 已形成的设计文档默认视为冻结，正常情况不再直接修改 `doc/vCoding后台管理系统设计/` 下的既有设计文档。
- 若出现新增需求、需求调整、范围变更或设计偏差修正，统一在 `doc/需求变更/` 下新增需求变更文档，不直接覆盖原设计文档。
- `doc/需求变更/README.md` 作为需求变更文档模板，后续新增需求变更文件时必须按该模板结构与要求编写。
- 需求变更文档必须说明本次为什么要变更，以及具体变更内容。
- 需求变更文档建议使用 `yyyyMMdd-变更主题.md` 命名，便于后续自动化任务识别、汇总与合并。
- 仅在自动化任务执行“合并需求设计”时，才允许将需求变更内容回写或合并到原设计文档。
