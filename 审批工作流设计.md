...
- `HTML + Tailwind CSS` 适合作为后台管理系统页面的基础实现方式，适合承载流程定义管理、流程发布、流程实例查询、审批处理等页面
- `LogicFlow` 适合作为审批流程图设计器，用于实现节点拖拽、连线绘制、属性配置、条件分支配置等能力，并能较自然地映射到 `tb_workflow_node`、`tb_workflow_transition`、`tb_workflow_node_approver` 这套自定义表结构

### 2.3 流程图工具选型建议

本方案更推荐使用 `LogicFlow` 绘制审批流程图。
...
因此，本项目建议优先采用：`HTML + Tailwind CSS + LogicFlow` 作为流程设计与展示页面方案。

### 2.4 `HTML + Tailwind CSS + LogicFlow` 集成建议

#### 2.4.1 页面职责划分

建议采用“后台管理页使用 `HTML + Tailwind CSS` 构建，流程画布由前端 JS 初始化 `LogicFlow`”的组合方式。
...
其中：
- `HTML + Tailwind CSS` 负责页面骨架、表单区域、属性面板区域、按钮权限控制、基础数据展示
...
采用 `HTML + Tailwind CSS + LogicFlow` 的组合，适合当前项目的原因主要有：
- 后台管理系统页面多数是表格、表单、详情页，使用 `HTML + Tailwind CSS` 足够承载
- 流程设计器只是其中一个页面能力，没有必要为了一个设计器页面整体切换为重前端架构
- `LogicFlow` 可以专注解决流程图绘制与交互，而 `Spring Boot 3` 继续负责业务接口与页面组织
- 对当前项目而言，这种方案复杂度更低，交付更稳妥
...
- 基于 `HTML + Tailwind CSS` 的后台页面可负责流程列表、版本管理、设计器入口、审批详情等管理页面，流程图本身由前端嵌入 `LogicFlow` 画布实现
...
| 接口路径 | 方法 | 说明 |
|----------|------|------|
| `/sys/auth/login` | POST | 使用用户名密码登录并签发 Bearer Token |
| `/sys/auth/current-user` | GET | 获取当前登录用户基础上下文、角色编码和权限标识 |

当前阶段认证实现说明：
- 认证框架统一使用 `Spring Security`
- 登录态统一使用无状态 `Bearer Token`
- 请求头统一传递：`Authorization: Bearer {token}`
- Token 签名密钥统一通过 `workflow.security.token-secret` 配置
